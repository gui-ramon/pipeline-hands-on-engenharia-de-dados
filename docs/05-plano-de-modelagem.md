...
| **Random Forest** | Lida nativamente com a mistura de categórica+numérica das 15 features (após encoding), robusto a overfitting com floresta grande, dá feature importance própria pra comparar com o V de Cramér já medido na EDA. **Custo controlado via `max_samples`** (cada árvore vê no máximo ~300 mil linhas por bootstrap, não o treino inteiro) — é o modelo mais pesado dos três em RAM/tempo numa floresta de milhões de linhas; sem esse teto, `n_estimators` árvores tentando crescer sobre a base inteira de treino esgotaria memória num notebook comum. Ver `src/modelagem/treino.py::treinar_modelos`. |
